...
// SQL query to fetch products for orders, including TINNumber and VRN from the customers table
$sql = "
    SELECT 
        o.ID AS OrderID, 
        o.UserID, 
...
$sql .= " ORDER BY o.ID DESC"; // Orders by the latest OrderID first
...
if ($result && $result->num_rows > 0) {
    ...
    foreach ($orders as $orderId => $orderDetails) {
        ...
        $tinNumber = $orderDetails[0]['TINNumber'];
        $vrn = $orderDetails[0]['VRN'];
        $lpo = $orderDetails[0]['LPO'];
...
<ul class="list-group">
    <li class="list-group-item border-0 d-flex justify-content-between ps-0 mb-2 border-radius-lg">
        <div class="d-flex flex-column">
            <h6 class="mb-1 text-dark font-weight-bold text-sm"><?php echo $customerName; ?></h6>
            <span class="text-xs">TIN: <?php echo $tinNumber; ?> | VRN: <?php echo $vrn; ?></span><br>
            <span class="text-xs"><?php echo $datePurchased; ?></span>
        </div>
        <div class="d-flex align-items-center text-sm">
            Tzs <?php echo $totalCost; ?>
            <a href="../invoice/invoice.php?OrderID=<?php echo $orderId; ?>&customerName=<?php echo urlencode($customerName); ?>&tinNumber=<?php echo urlencode($tinNumber); ?>&vrn=<?php echo urlencode($vrn); ?>&products=<?php echo urlencode($products); ?>&totalCost=<?php echo $totalCost; ?>&paymentMethod=<?php echo urlencode($paymentMethod); ?>" class="btn btn-link text-dark text-sm mb-0 px-0 ms-4">
